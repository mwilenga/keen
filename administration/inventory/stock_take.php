  ...
  require_once('../dBConfig/dBConnect.php');
    ...
    if(isset($_POST['submit'])){
      ...
      $check = mysql_query("select * from tms_items where item_name = '$name'");
                 ...
                 $ded = $ch['qnty'];
                 $total = $tr * $qnty;
	  $in = "insert into tms_stock_take values('','$name','$qnty','$total','$date','$mon','$year')";
	   $insert = mysql_query($in);
			   if($insert){
                
                 $income = mysql_query("insert into tms_income values('','$date','$total','$mon','$year','$name')");
                 if($income){
                   $update = $ded - $qnty;
