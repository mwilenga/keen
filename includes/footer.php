 <footer id="footer">
    <div class="footer_top">
      <div class="row">
        <div class="col-lg-4 col-md-4 col-sm-4">
          <div class="footer_widget wow fadeInLeftBig">
            <h2>KEEN COMPUTERS</h2>
            <p>Business Name</p>
            <address>
            <b>Keen Communications Limited</b>
            </address>
            <p>Business type</p>
            <address>
            <b>Company</b>
            </address>

          </div>
        </div>
        <div class="col-lg-4 col-md-4 col-sm-4">
          <div class="footer_widget wow fadeInDown">
            <h2>FAST LINKS</h2>
            <ul class="tag_nav">
              <li><a href="about.php">About us</a></li>
              <li><a href="#">Contact us</a></li>
              <li><a href="products.php">Our product</a></li>
              <li><a href="our_services.php">Our services</a></li>
              <li><a href="our_clients.php">Our clients</a></li>
            </ul>
          </div>
        </div>
        <div class="col-lg-4 col-md-4 col-sm-4">
          <div class="footer_widget wow fadeInRightBig">
            <h2>Contact</h2>
            <p>E-mail: </p>
            <address>
            <b>info@keencommunications.co.tz</b>
            </address>
            <p>Tel</p>
             <address>
            <b>+255 765 542 573 / +255 713 747 904</b>
            </address>
            <p>Physical Address:</p>
             <address>
            <b>Survey Road (DDC Park Mlimani city)</b>
            </address>
            <p>Postal Addres</p>
             <address>
            <b>80329  Dar es salaam</b>
            </address>
            
          </div>
        </div>
      </div>
    </div>
    <div class="footer_bottom">
      <p class="copyright">Copyright &copy; 2017 <a href="index.html">Keen computers</a></p>
      <p class="developer">Developed By Tuwasiliane Limited</p>
    </div>
  </footer>
</div>
<script src="assets/js/jquery.min.js"></script> 
<script src="assets/js/wow.min.js"></script> 
<script src="assets/js/bootstrap.min.js"></script> 
<script src="assets/js/slick.min.js"></script> 
<script src="assets/js/jquery.li-scroller.1.0.js"></script> 
<script src="assets/js/jquery.newsTicker.min.js"></script> 
<script src="assets/js/jquery.fancybox.pack.js"></script> 
<script src="assets/js/custom.js"></script>
</body>
</html>

